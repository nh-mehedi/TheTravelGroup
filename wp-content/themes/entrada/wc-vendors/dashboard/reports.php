...
<table class="table table-condensed table-vendor-sales-report">
	...
	<?php if ( !empty( $vendor_summary ) ) : ?>
			...
			<?php foreach ( $vendor_summary[ 'products' ] as $product ) :
				...
				<tr>
					...
					<?php if ( $can_view_orders ) : ?>
						<td>
							<a href="<?php echo $product[ 'view_orders_url' ]; ?>"><?php _e( 'Show Orders', 'entrada' ); ?></a>
						</td>
					<?php endif; ?>
		...
		<?php else : ?>
			...
			<tr>
				<td colspan="4"
					style="text-align:center;"><?php _e( 'You have no sales during this period.', 'entrada' ); ?></td>
			</tr>
	...
	<?php else : ?>
		...
		<tr>
			<td colspan="4"
				style="text-align:center;"><?php _e( 'You haven\'t made any sales yet.', 'entrada' ); ?></td>
		</tr>
